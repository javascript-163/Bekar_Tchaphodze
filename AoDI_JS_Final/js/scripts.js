        ...
        fetch(apiUrl)
          ...
          .then(data => {
            ...
            data.events.forEach(event => {
              ...
              eventVenue.textContent = `Venue: ${event.location.name}, ${event.location.address.addressLocality}, ${event.location.address.addressCountry.name}`;
              eventDiv.appendChild(eventVenue);

              const eventDate = document.createElement('p');
              eventDate.textContent = `Date: ${new Date(event.startDate).toLocaleDateString()}`;
